...
def receive():
    threading.Timer(1/30, receive).start()
    message = client.recv(1024).decode()
    if message == 'NICK':
        client.send(bytes(nickname, "utf-8"))

    elif (msg := message.split(","))[0][5:] != nickname:
        if other_players[msg]:
            other_players[msg[0][5:]].position = (float(msg[1][2:]), float(msg[2][2:]), float(msg[3][2:]))

        else:
            other_players[msg] = Entity(model="cube", color=rgb(random.randrange(255), random.randrange(255),
                                                                random.randrange(255)))


# Sending Messages To Server
def send_info():
    threading.Timer(1/30, send_info).start()
